...
主に ECS / CodePipeline を用いたコンテナ基盤の設計・構築経験があります。  
...
フロントとして顧客と自社の調整をつけ、コミュニケーション推進を行った経験があります。
...
### 顧客情報認証システム用コンテナ基盤技術支援（2022/11 - 2023/4)

- 概要  
  - 顧客情報を利用するための認証システムのフロントエンド、バックエンドのコンテナ化に伴う技術支援を実施。  
...
- 環境
  - AWS サービス： VPC / ELB(ALB・NLB) / ECS / EFSRDS(Aurora) / ElastiCache(Redis) / S3 / CloudFront / WAF / ECS(Fargate) / ECR / API Gateway / CodeCommit / CodeBuild / CodeDeploy / CodePipeline / EventBridge
  - IaC： CloudFormation

### 社内システム用マルチアカウント基盤技術支援（2022/7 - 2023/3)

- 概要  
  - 社内システム用のマルチアカウント管理基盤の設計/構築に伴う技術支援を実施。  
...
- 概要  
  - Web サイトリニューアルに伴い、RDS インスタンスタイプを定期的に自動変更するためのシステムを構築。  
...
- 環境
  - AWS サービス：IAM / CloudFront / S3 / Lambda(Node.js) / Route53 / ACM
  - IaC： Terraform

### CMSインフラ用AWS環境構築（2022/5 - 2022/8)

- 概要  
  - Web サイトリニューアルに伴い、動画サイド構築 CMS 新規導入のためのインフラ基盤を構築。  
...
- 環境
  - AWS サービス：IAM / VPC / ELB(ALB) / EC2 / RDS(MySQL) / ElastiCache(Redis) / S3 / Route53 / ACM
  - IaC： Terraform
...
### 情報システム用vSpherem基盤更改（2021/10 - 2021/12)
...
- [linebot-yubaba](https://github.com/tsukuboshi/linebot-yubaba)
  - 自作の Lambda と Line Messaging API を活用した湯 婆 婆 bot のコードを格納したリポジトリ
...
- [DevelopersIO 2022](https://dev.classmethod.jp/news/devio-2022/) (2022/7/26-28)
